...
#%% Print the number of unique students and length of the dataframe by student
...
#%% Print the length of unique values in each column
cols = oli.columns
# This won't work because wrong column name is used
stud_ids = oli['Anon Student Id']
all_studs = pd.unique(stud_ids)

for col in cols:
    print(len(pd.unique(oli[col])),' unique ', col)
#%% Print the unique problem names
prob_names = pd.unique(oli['Problem Name'])
print(prob_names)
#%% Print the unique corrects, incorrects, hints, and knowledge concepts
print('Corrects')
...
#%% Plot corrects, incorrects, and hints
plt.figure(1)
